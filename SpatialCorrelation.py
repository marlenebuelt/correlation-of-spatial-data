...
#inspired by: https://builtin.com/data-science/exploratory-spatial-data-analysis-esda, https://towardsdatascience.com/exploratory-spatial-data-analysis-esda-spatial-autocorrelation-7592edcbef9a
...
df = data.loc[:2000,['munic','ETHANOLrp','longitude', 'latitude']].dropna()
...
munic_map.plot(ax=ax, color='lightgrey')
geo_df.plot(ax=ax)
...
munic_map2 = gpd.read_file('/Users/marlenebultemann/Desktop/HTW/UM/correlation-of-spatial-data/BR_Municipios_2020/BR_Municipios_2020.shp')
munic_map2.plot()
munic_map2.to_crs(epsg=4326).plot()
...
fig, ax = plt.subplots(figsize = (10,10))
munic_map2.to_crs(epsg=4326).plot(ax=ax, color='red')
munic_map2.plot(ax=ax, color='black')
geo_df2.plot(ax=ax)
ax.set_title('Municipalities_drop')

fig, ax = plt.subplots(figsize = (10,10))
munic_map2.to_crs(epsg=4326).plot(ax=ax, color='pink')
